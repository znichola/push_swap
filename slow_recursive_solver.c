...
/*   Updated: 2022/12/09 22:55:27 by znichola         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	slow_recursive_solver(t_app *a)
{
	...
	if (a->s.o.c - a->s.o.rec_root + 1 == a->max_depth)
		return (FAILURE);
	...
	while (++i < OPS_NUM)
	{
		...
		if (a->opp_mod[i](&a->s) == SUCCESS)
		{
			...
			// if (DEBUG_TEST)
			// {
				if (a->s.o.c - a->s.o.rec_root + 1 != a->max_depth - 1)
					ft_printf("\n");
				for (int i = 0; i < a->s.o.c - a->s.o.rec_root + 1; i++)
					ft_printf(" ");
				if (a->s.o.c - a->s.o.rec_root + 1 != a->max_depth - 1)
					ft_printf("%d: ", a->s.o.c - a->s.o.rec_root + 1);
				write_op_row(i);
			// }
...
int	slow2(t_app *a)
{
	...
	if (i != a->s.size)
		return (FAILURE);
	while (i-- > 1)
		if (a->s.root_a[i] > a->s.root_a[i - 1])
			return (FAILURE);
	// checker
